...
class Agent(cellular.Agent):
    def __init__(self, world, vc):
        self.ai = v_est.V_est(
            range(directions), world, epsilon=1., alpha=0.1, gamma=0.5, vc=vc)
...
for vc in [0, 100, 10000]:
    ...
    for i in range(pretraining):
        if i % 100000 == 0:
            ...
            pp(v_est.sigmoid(agent.ai.betas.squeeze().T))
            print(agent.ai.v.T)
            agent.score = 0
            agent.deads = 0
        world.update()
    ...
    np.save('beta_maps/%d' % vc, v_est.sigmoid(agent.ai.betas.squeeze().T))
